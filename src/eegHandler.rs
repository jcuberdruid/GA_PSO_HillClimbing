...
fn load_array_from_file<P: AsRef<Path>>(path: P) -> io::Result<Vec<Vec<f64>>> {
    ...
    let n = nums.len() / 10240; // Adjust 'n' as needed
    let arrays = (0..n).map(|i| nums[i*10240..(i+1)*10240].to_vec()).collect();
